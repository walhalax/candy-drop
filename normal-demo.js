// ===== ふつうモード デモ用 ===== (basic 4-match tutorial with 2 patterns)
(function() {
  const demoCanvas = document.getElementById('normal-demo-canvas');
  ...
  let chainLevel = 1;
  let currentPattern = 1; // 1 or 2

  // ===== パターン1: 基本4連結 =====
  const DUR1 = { idle: 1500, falling: 2500, burst: 500, final: 2000 };
  ...
  function initPattern1() {
    ...
    board[2][1] = CANDY_RED;
    // 落下ペア: 赤+黄
    fallingPair = { x: 1, y: 0, colors: [CANDY_RED, CANDY_YELLOW] };
    burstingCells = [];
    burstTimer = 0;
    instantFlash = 0;
    flashIntensity = 0;
  ...
  function updatePattern1(delta) {
    ...
    switch (demoPhase) {
      case 'idle':
        if (demoPhaseTimer >= DUR1.idle) { demoPhase = 'falling'; demoPhaseTimer = 0; }
        break;
      case 'falling':
        fallingPair.y += delta * 0.0018;
        if (fallingPair.y >= 1) {
          // 赤をcolumn 1に、黄をcolumn 2に配置 -> 赤4連結完成
          board[1][1] = fallingPair.colors[0];
          board[1][2] = fallingPair.colors[1];
          fallingPair = null;
          demoPhase = 'burst';
          demoPhaseTimer = 0;
        }
  ...
  // ===== パターン2: 2連鎖 =====
  // 初期配置:
  // 空空🟢🟣 (row 0) - GREEN=col2, PINK=col3
  // 空空空空 (row 1)
  // 空空🟢🟣 (row 2) - GREEN=col2, PINK=col3
  // 🔴空🟣🟣 (row 3) - PINK=col2, PINK=col3
  // 🔴🟢🟢🟠 (row 4) - GREEN=col2, GREEN=col3
  //
  // 落下: GREEN+PINK が column 2から落ちて PINK4連結 (1連鎖目)
  // → PINK消除後、重力でGREEN落下 → GREEN4連結 (2連鎖目)
  const DUR2 = { idle: 1200, falling: 2200, burst1: 500, settling: 700, burst2: 500, final: 2000 };

  function initPattern2() {
    board = Array(ROWS).fill(null).map(() => Array(COLS).fill(null));
    // Row 0: 空空🟢🟣
    board[0][2] = CANDY_GREEN;
    board[0][3] = CANDY_PINK;
    // Row 2: 空空🟢🟣
    board[2][2] = CANDY_GREEN;
    board[2][3] = CANDY_PINK;
    // Row 3: 🔴空🟣🟣
    board[3][0] = CANDY_RED;
    board[3][2] = CANDY_PINK;
    board[3][3] = CANDY_PINK;
    // Row 4: 🔴🟢🟢🟠
    board[4][0] = CANDY_RED;
    board[4][1] = CANDY_GREEN;
    board[4][2] = CANDY_GREEN;
    board[4][3] = CANDY_ORANGE;
    // 落下ペア: GREEN + PINK (column 2から。satelliteはcol3)
    fallingPair = { x: 2, y: 0, colors: [CANDY_GREEN, CANDY_PINK] };
    burstingCells = [];
    burstTimer = 0;
    instantFlash = 0;
    flashIntensity = 0;
  ...
  function updatePattern2(delta) {
    ...
    switch (demoPhase) {
      case 'idle':
        if (demoPhaseTimer >= DUR2.idle) { demoPhase = 'falling'; demoPhaseTimer = 0; }
        break;
      case 'falling':
        fallingPair.y += delta * 0.0018;
        if (fallingPair.y >= 1) {
          // 落下元のブロックをクリア（着地先に移動した）
          board[0][2] = null;
          board[0][3] = null;
          // GREENがcol2に、PINKがcol3に着地
          // → PINK4連結完成 (col3: row0, row2, row3, row1=新)
          board[1][2] = fallingPair.colors[0]; // GREEN -> (1,2)
          board[1][3] = fallingPair.colors[1]; // PINK -> (1,3)
          fallingPair = null;
          demoPhase = 'burst1';
          demoPhaseTimer = 0;
        }
        break;
      case 'burst1':
        if (demoPhaseTimer >= DUR2.burst1) {
          const cleared = checkMatches();
          if (cleared) {
            burstingCells = cleared;
            burstTimer = 0;
            chainLevel = 1;
            triggerEffects(cleared);
          }
          demoPhase = 'settling';
          demoPhaseTimer = 0;
        }
        break;
      case 'settling':
        if (demoPhaseTimer >= DUR2.settling) {
          applyGravity();
          demoPhase = 'burst2';
          demoPhaseTimer = 0;
        }
  ...
  function triggerEffects(cleared) {
    instantFlash = 0.4;
    instantFlashColor = rainbowColors[chainLevel - 1] || rainbowColors[0];
    flashIntensity = 0.5 + chainLevel * 0.1;
    centerGlow = 0.3 + chainLevel * 0.1;
    cleared.forEach(([cx, cy]) => {
      shockwaves.push({ x: cx * CELL + CELL / 2, y: cy * CELL + CELL / 2, radius: 0, maxRadius: CELL * (3 + chainLevel * 0.5), alpha: 0.7 + chainLevel * 0.05 });
    });
  }
  ...
  function drawDemo() {
    ...
    for (let y = 0; y < ROWS; y++) {
      for (let x = 0; x < COLS; x++) {
        if (board[y][x]) {
          const isObstacle = board[y][x] === 'obstacle';
          drawWrappedCandy(ctx, x * CELL, y * CELL, board[y][x], CELL, 1, 0, isObstacle);
        }
      }
    }
    ...
    if (fallingPair) {
      ...
      if (displayY >= 0 && displayY < ROWS) {
        drawWrappedCandy(ctx, fallingPair.x * CELL, displayY * CELL, fallingPair.colors[0], CELL, 0.8, 0, false);
        drawWrappedCandy(ctx, (fallingPair.x + 1) * CELL, displayY * CELL, fallingPair.colors[1], CELL, 0.8, 0, false);
      }
    }
  }
